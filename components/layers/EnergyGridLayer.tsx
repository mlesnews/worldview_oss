...
const LAYER_COLOR = "#ffaa00";

export default function EnergyGridLayer({ viewer }: Props) {
  const { items } = useEnergyGrid(true);
  const dsRef = useRef<Cesium.CustomDataSource | null>(null);
  const setSelectedEntity = useWorldViewStore((s) => s.setSelectedEntity);
  const flyTo = useWorldViewStore((s) => s.flyTo);

  useEffect(() => {
    const ds = new Cesium.CustomDataSource("energyGrid");
    ...
    handler.setInputAction(
      (click: { position: Cesium.Cartesian2 }) => {
        ...
        if (Cesium.defined(picked) && picked.id?._energyData) {
          const d = picked.id._energyData;
          setSelectedEntity({
            id: d.id,
            type: "news",
            name: d.name,
            details: {
              Name: d.name,
              Type: d.type,
              "Capacity (MW)": d.capacity,
              "Current Output": d.currentOutput,
              Region: d.region,
            },
            lon: d.longitude,
            lat: d.latitude,
  ...
  useEffect(() => {
    ...
    for (const item of items) {
      ...
      (entity as unknown as Record<string, unknown>)._energyData = item;
    }
  }, [items]);
